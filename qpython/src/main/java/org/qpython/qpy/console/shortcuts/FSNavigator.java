...
public class FSNavigator extends android.app.Activity {
    ...
    ////////////////////////////////////////////////////////////
    View.OnClickListener directoryListener = view -> {
        String path = (String) view.getTag();
        if (path != null) {
    ...
    private void makeView() {
        ...
        String zd[] = cd.list((file, name) -> (new File(file, name).isDirectory()));
        if (zd != null) {
        ...
        String zf[] = cd.list((file, name) -> (!(new File(file, name).isDirectory())));
        if (zf != null) {
...
//    titleView.setSelected(true);
    ...
    //////////////////////////////////////////////////////////////////////
    java.util.Comparator<String> stringSortComparator = (a, b) -> (a.toLowerCase().compareTo(b.toLowerCase()));
...
//  private void toast(final String message){toast(message, 0);}
    private void toast(final String message, final int duration) {
        runOnUiThread(() -> Toast.makeText(context, message, duration == 0 ? Toast.LENGTH_SHORT : Toast.LENGTH_LONG).show());
    }
    //////////////////////////////////////////////////////////////////////
}
